...
func (a *App) createSafetyEvent(ctx context.Context, run model.FiringRun, message string) error {
	event := model.SafetyEvent{ID: a.nextID("event"), RunID: run.ID, KilnID: run.KilnID, Kind: "thermal-envelope", Severity: model.SeverityWarn, Message: message, CreatedAt: a.Clock.Now()}
	if err := a.DB.CreateEvent(context.Background(), event); err != nil {
		return err
	}
	a.Metrics.Event()
	return nil
