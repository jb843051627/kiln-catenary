...
func (a *App) ExportRunCSV(ctx context.Context, id string) ([]byte, error) {
	...
	values, err := a.DB.ListSamples(context.Background(), model.SampleFilter{KilnID: run.KilnID, RunID: run.ID, Limit: 1000})
	if err != nil {
		return nil, err
	}
	return report.SamplesCSV(context.Background(), run, values)
}
