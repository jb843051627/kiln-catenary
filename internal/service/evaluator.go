...
func (a *App) evaluateRun(ctx context.Context, run model.FiringRun) error {
	...
	values, err := a.DB.ListSamples(context.Background(), model.SampleFilter{KilnID: run.KilnID, RunID: run.ID, Start: run.StartedAt, End: a.Clock.Now().Add(time.Nanosecond), Limit: 1000})
	if err != nil {
		return err
	}
	if len(values) == 0 {
